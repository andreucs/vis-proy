...
# Supongamos que tu dataset ya está cargado
data <- ocup_nacional

# Añade una columna de fecha (si no está incluida ya)
data$Fecha <- as.Date(paste(data$AÑO, data$MES, "01", sep = "-"))

# Crear el gráfico principal con las líneas de pernoctaciones
fig <- plot_ly(data, x = ~Fecha, y = ~pernoc, type = 'scatter', mode = 'lines', name = "Pernoctaciones", showlegend=F)

# Añadir las líneas verticales y trazas invisibles para la leyenda
fig <- fig %>%
  
  # Línea sólida roja: Estado de alarma (14 de marzo de 2020)
  add_trace(
  ...
  # Línea discontinua azul: Fin del estado de alarma (21 de junio de 2020)
  add_trace(
  ...
  # Línea guiones verde: Nuevo estado de alarma (25 de octubre de 2020)
  add_trace(
  ...
  # Línea guiones-puntos púrpura: Fin de la crisis sanitaria (5 de julio de 2023)
  add_trace(
  ...
  layout(
    ...
    yaxis = list(title = "Pernoctaciones",
                 range=c(0, 120e6)),
    legend = list(
      x = 0.01, # Posición horizontal (0 es extremo izquierdo)
      y = 0.99, # Posición vertical (1 es extremo superior)
      bgcolor = "rgba(255,255,255,0.7)", # Fondo blanco semitransparente para la leyenda
      bordercolor = "black",
      borderwidth = 1,
...
```{r}
# Ajustar los datos reales para alinearlos con las fechas de predicción
data1_filtrado <- data1 %>%
  filter(Fecha %in% pred$fecha)  # Filtrar solo las fechas que coinciden con las predicciones

# Unir las predicciones y los valores reales
datos_combinados <- merge(pred, data1_filtrado, by.x = "fecha", by.y = "Fecha", all.x = TRUE)

# Calcular ymin y ymax (área sombreada)
