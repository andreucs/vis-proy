...
get_tableau_ranking <- function(df_prov, df_ranking_provinces, min_year = 2019, max_year = 2023) {
    ...
    for (year in min_year:max_year) {
        rank_df_year <- data.table(
            PROVINCIA_DESTINO = df_ranking_provinces[[year]],
            RANK = 1:nrow(df_ranking_provinces)
        )
    ...
    all_rank_year <- all_rank_year[order(get(paste0("RANK_", min_year)))]
...
get_bumpchart_ranking <- function(df_ranking_tableau) {
    ...
    bump_format[, Position := n_elements - Position]
